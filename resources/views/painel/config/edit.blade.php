...
<title>SpartumWEB - Configurações</title>
	...
	<body>
                ...
                <div class="content">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <input type="text" id="edit_cnpj" class="form-control input-lg" value="{!! Helpers::mask($config->CNPJ_UTILIZADOR, '##.###.###/####-##') !!}" disabled>
                        </div>
		...
		{{ csrf_field() }}
            ...
            {{-- CRIANDO OS INPUT HIDDEN PARA LER NO JQUERY E MONTAR NA PRIMEIRA LEITURA (MODO DEFAULT) --}}
            <input type="hidden" id="hidden_usa_pedido_grade"   value="{!! $config->FLG_USA_GRADE_PEDIDO !!}">
            <input type="hidden" id="hidden_imp_pedido_grade"   value="{!! $config->FLG_IMP_PEDIDO_GRADE !!}">
            <input type="hidden" id="hidden_imp_tam_modo_lista" value="{!! $config->FLG_IMP_TAM_MODO_LISTA !!}">
            <input type="hidden" id="hidden_prazo_tab_externa"  value="{!! $config->FLG_PRAZO_PAGTO_TABELA_EXTERNA !!}">
            ...
            <div class="form-group-style">
                ...
                <table class="table">
                    ...
                    <tr>
                        <td width="80%"><strong>Limpar campos dos dados dos PRODUTOS no ato da inclusão no pedido?</strong>
                                   </br>Deixando esta opção marcada, quando inclui-se um item no pedido, o sistema limpará os campos e setará para o campo inicial, para começar uma nova inclusão, se você não utiliza de grade, é recomendado que deixe esta opção ativada.
                        </td>
                        <td width="20%">
                            @if ( $config->FLG_LIMPA_CAMPOS_ADD_ITEM == 0 )
                            <input type="checkbox" id="flag_limpa_campos_adicao_item" data-toggle="toggle" data-on="Sim" data-off="Não" name="flag_limpa_campos_adicao_item" data-onstyle="primary" data-offstyle="danger">
                            @else 
                            <input type="checkbox" id="flag_limpa_campos_adicao_item" data-toggle="toggle" data-on="Sim" data-off="Não" name="flag_limpa_campos_adicao_item" data-onstyle="primary" data-offstyle="danger" checked>
                            @endif                     
                        </td>
                    </tr>                    
                    ...
                    <tr>
                        <td width="80%"><strong>Utilizar de GRADES na venda de ITENS do PEDIDO?</strong>
                                   </br>Esta opção faz com que o sistema utilize de uma tabela de grade de tamanhos para os produtos, desmarcando esta opção o sistema irá desmarcar também as opções de impressão que envolvem GRADE.
                    ...
                    <tr>
                        <td width="80%"><strong>Usar PRAZOS DE PAGAMENTO de uma tabela externa?</strong>
                                   </br>Esta opção faz com que o sistema utilize de uma tabela externa para exibir os prazos cadastrados num sistema retaguarda.
                    ...
                    <tr>
                        <td width="80%"><strong>Usar impressão de pedidos em formato GRADE?</strong>
                                   </br>Esta opção faz com que a impressão de pedidos mostre os tamanhos dos produtos nas colunas, logo após a descrição do item.
                    ...
                    <tr>
                        <td width="80%"><strong>Mostrar coluna de TAMANHO para impressão de pedido?</strong>
                                   </br>Esta opção faz com que a impressão de pedidos mostre ou não, os tamanhos dos produtos em modo lista.</td>
